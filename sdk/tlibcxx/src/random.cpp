...
// Priority TBD.
// Initially, we planned to support random using sgx_read_rand.
// Then we decided not to support it until we understand
// whether it's used correctly elsewhere, see HSD 205844.
#include <__config>
...
#elif defined(_LIBCPP_USING_SGX_RANDOM)
#include "sgx_trts.h"
#endif
...
{
    unsigned r;
    if (SGX_SUCCESS != sgx_read_rand(reinterpret_cast<unsigned char*>(&r), sizeof(r))) {
        __throw_system_error(EINVAL, "random_device sgx_read_rand failed.");
    }
    return r;
}
