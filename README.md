...
See the later topic, *Install Intel(R) SGX PSW*, on for information on installing the PSW package.
- Copy the code samples installed by the Intel(R) SGX SDK package into your work folder, for example:   
```
  $ cp -r /opt/intel/sgxsdk/SampleCode ~
```
- Compile and run each code sample in Hardware mode, Bebug build, as follows:  
```
  $ cd SampleCode/LocalAttestation
  $ make
  $ ./app
