...
def run_xed(input, mod, addr, inlen):
    # run it
    ret = subprocess.run('$XED_BIN/xed -64 -mpx -cet  -v 5 -j -de {}'.format(input), shell= True, stdout=subprocess.PIPE)
    stdout = ret.stdout.decode("utf-8")
    if ret.returncode != 0:
    ...
    inst_len = 0
    imm_len = 0
    string_instruction = False
    for l in stdout.splitlines():
        if 'CATEGORY:   X87_ALU' in l or 'EXTENSION:  AVX' in l or 'ICLASS:     NOP' in l or 'ICLASS:     UD' in l:
            assert(mod == 0)
            return
        if 'CATEGORY:' in l and 'STRINGOP' in l:
            string_instruction = True
    ...
    #print(inst_len, inlen, imm_len)    
    l = ls[4]
    passed = addr in l
    # check memory access type
    if l.startswith('PUSH'):
        assert(mod == 2 and addr =='[RSP]' and inlen == inst_len - imm_len)
    elif l.startswith('CALL'):
        if 'POS_NOMINAL_OPCODE:' not in l:
            # a special case when the first byte is opcode
            assert(mod == 2 and addr =='[RSP]' and inlen == 1)
            return
        st_idx = l.find('POS_NOMINAL_OPCODE:')
        ed_idx = l[st_idx:].find(',')
        opcode_pos = int(l[st_idx+19: ed_idx + st_idx])
        st_idx = l.find('NOMINAL_OPCODE:')
        ed_idx = l[st_idx:].find(',')
        opcode = int(l[st_idx+15: ed_idx + st_idx])
        if opcode == 255:
            inst_len = opcode_pos + 2
        else:
            inst_len = opcode_pos + 1
        assert(mod == 2 and addr =='[RSP]' and inlen == inst_len)
    elif string_instruction:
...
def test_1opcode(op1):
    ...
    for rex in rexs:  
        for modrm in range(0, 257):
            ans.append( rex + op1 + map[modrm])
    for rex in rexs:
        for h in range(0, 4):
            for modrm in range(0, 8):
                t = modrm | (h<<6)
                for sib in range(0, 33):
                    ans.append(rex + op1 + map[t] + map[sib])
    return ans
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        for op in range(0, 16):
            ...
            if len(op1)<2:
                op1 = '0'+op1
            for input in test_1opcode(op1):
                for init in ['', 'F0', 'F02E', 'F066', 'F03666', 'F02667','F26467', 'F22E66','F2366667','F326','F36466','F36567','F26566', 'F064','F06566']:
                    total += 1
                    temp = init + input+imm4
